...
BoxDecoration boxDecorations({double radius = 8, Color color = Colors.transparent, Color bgColor = Colors.white, var showShadow = false}) {
  return BoxDecoration(
    color: bgColor,
    boxShadow: showShadow ? [BoxShadow(color: chShadowColor, blurRadius: 10, spreadRadius: 2)] : [BoxShadow(color: Colors.transparent)],
    border: Border.all(color: color),
    borderRadius: BorderRadius.all(Radius.circular(radius)),
  );
}

BoxDecoration boxDecoration({double radius = 80.0, Color backGroundColor = chPrimaryColor, double blurRadius = 8.0, double spreadRadius = 8.0, Color radiusColor = Colors.black12, Gradient gradient}) {
  return BoxDecoration(
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: widget.isStroked ? boxDecorations(bgColor: Colors.transparent, color: chPrimaryColor) : boxDecorations(bgColor: chPrimaryColor, radius: 6),
      ),
    );
  }
}
